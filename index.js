...
function manipulateSearchParams(paramsObj, newUrl) {
  ...
  for (let searchKey of key) {
    urlObj.searchParams.append(searchKey, paramsObj[searchKey]);
  }
...
function deleteSearchParams(keys, url) {
  ...
  for (let searchKey of keys) {
    if (urlObj.searchParams.has(searchKey)) {
...
function manipulateQuery(url, options) {
  ...
  if (options.has(`append`)) {
    ...
    for (let [key, value] of appendOptions) {
      urlObj.searchParams.append(key, value);
    }
  }
...
function sortUrlParams(url) {
  ...
  let sortParam = paramsArray.sort((a, b) => a.key - b.key);
  // Очищуємо пошукові параметри URL.
  paramsArray.clear;
  // Додаємо відсортовані параметри до URL.
  urlObj.searchParams.has(sortParam);
  // Повертаємо новий URL як рядок.
  return urlObj.toString();
}
